...
#define END				"sprite/gameover.png"
...
static void bomb_load() {
	bomb[BOMB1] = image_load(BOMB_TTL4);
	bomb[BOMB2] = image_load(BOMB_TTL3);
	bomb[BOMB3] = image_load(BOMB_TTL2);
	bomb[BOMB4] = image_load(BOMB_TTL1);
	bomb[EXPLO] = image_load(EXPLOSION);
}
...
SDL_Surface* sprite_get_bomb(enum bomb_type bomi){
	assert(bomb[bomi]);
	return bomb[bomi];
}
